...
[Ubuntu 22.04 LTS (Jammy Jellyfish)](https://releases.ubuntu.com/jammy/) (Linux) for integration testing.
...
The sources are available at the [`foundata/oci-ubuntu2204-itt` repository](https://github.com/foundata/oci-ubuntu2204-itt). To build the image locally, do the following:
...
2. Use the image you built earlier or pull the image from a registry:
   - [Quay](https://quay.io/repository/foundata/ubuntu2204-itt):
     ...
     ```
   - [Docker Hub](https://hub.docker.com/r/foundata/ubuntu2204-itt):
     ```bash
     podman pull docker.io/foundata/ubuntu2204-itt:latest
     ```
3. Run a container from the image:
   ```bash
   podman run --detach --systemd=always ubuntu2204-itt:latest "/lib/systemd/systemd"
   ```
